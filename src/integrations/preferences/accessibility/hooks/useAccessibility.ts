...
function showImages() {
  ...
  placeholders.forEach((placeholder) => {
    const img = hiddenImagesMap.get(placeholder);
    if (img) {
      img.style.display = "";
  ...
  // Also restore any images that might have been hidden without placeholder
  document.querySelectorAll<HTMLImageElement>("img[data-a11y-hidden]").forEach((img) => {
    img.style.display = "";
    img.removeAttribute("data-a11y-hidden");
  });
}
